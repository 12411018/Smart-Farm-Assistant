...
def load_all_documents(data_dir: str) -> List[Any]:
    ...
    for csv_file in csv_files:
        ...
        try:
            # Use pandas to load CSV first to validate it
            import pandas as pd
            ...
            # Combine all columns into a single text for document content
            from langchain_core.documents import Document
            for idx, row in df.iterrows():
                # Create document content by combining all columns
                content = " | ".join([f"{col}: {str(val)}" for col, val in row.items()])
                doc = Document(
                    page_content=content,
                    metadata={"source": str(csv_file), "row": idx}
                )
                documents.append(doc)
            
            print(f"[DEBUG] Loaded {len(df)} CSV docs from {csv_file}")
        except Exception as e:
